...
function optimize(me::InferenceState)
    ...
    if me.optimize
        ...
        linearize_pass!(me)
        fold_constant_getfield_pass!(me)
        split_undef_flag_pass!(me)
...
function inline_as_constant(@nospecialize(val), argexprs, sv::InferenceState, @nospecialize(invoke_data))
    ...
    return (maybe_quoting(val), stmts)
end

function is_self_quoting(@nospecialize(x))
    return isa(x,Number) || isa(x,AbstractString) || isa(x,Tuple) || isa(x,Type) || isa(x,Function)
end
maybe_quoting(@nospecialize(x)) = is_self_quoting(x) ? x : QuoteNode(x)
...
function get_const_exprarg(@nospecialize(obj), sv)
    if isa(obj, Slot) || isa(obj, SSAValue) || isa(obj, Expr)
        return false, nothing
    elseif isa(obj, Symbol)
        if isdefined(sv.mod, obj) && isconst(sv.mod, obj)
            return true, getfield(sv.mod, obj)
        else
            return false, nothing
        end
    elseif isa(obj, GlobalRef)
        if isdefined(obj.mod, obj.name) && isconst(obj.mod, obj.name)
            return true, getfield(obj.mod, obj.name)
        else
            return false, nothing
        end
    elseif isa(obj, QuoteNode)
...
function fold_constant_getfield_pass!(sv::InferenceState)
    body = sv.src.code
    nexpr = length(body)
    for i in 1:nexpr
        ex = body[i]
        ...
        (head === :call && length(ex.args) >= 3) || continue
        is_known_call(ex, getfield, sv.src, sv.mod) || continue
        isconst, obj = get_const_exprarg(ex.args[2], sv)
        isconst || continue
        ...
        if isdefined(obj, fld)
            parent[pidx] = maybe_quoting(getfield(obj, fld))
        end
    end
end
...
function should_use_undef_flag(src::CodeInfo, id)
    var_has_undef(src, id) || return false
    return src.slottypes[id] != Void
end
...
function split_undef_flag_pass!(sv::InferenceState)
    body = sv.src.code
    len = length(body)
    next_i = 1
    ...
    while next_i <= len
        ...
        if isa(ex, Slot)
            id = slot_id(ex)
            should_use_undef_flag(sv.src, id) || continue
        ...
        if head === :(=)
            ...
            if isa(lhs, Slot)
                id = slot_id(lhs)
                if should_use_undef_flag(sv.src, id)
...
struct AllocOptContext
    ...
    function AllocOptContext(infomap::ValueInfoMap, sv::InferenceState)
        todo = ObjectIdDict()
...
function assign_update_todo!(ctx, assign::Expr)
    lhs = assign.args[1]
    if isa(lhs, SSAValue) || isa(lhs, Slot)
        add_allocopt_todo(ctx, lhs)
    end
end
...
function propagate_const_def!(ctx::AllocOptContext, info, key)
    var_has_undef(ctx.sv.src, key.first, key.second) && return false
    local constv
    for def in info.defs
        # No NewvarNode def for variables that aren't used undefined
        defex = (def.assign::Expr).args[2]
        isconst, v = get_const_exprarg(defex, ctx.sv)
    ...
    replace_ex = maybe_quoting(constv)
    ...
    for use in info.uses
            ...
            if use.exidx == 2 && is_known_call(use.expr, getfield, ctx.sv.src, ctx.sv.mod) &&
                ...
                if isa(fld, Symbol) || isa(fld, Int)
                    if isdefined(constv, fld)
                        ...
                        replace_use_expr_with!(ctx, use, maybe_quoting(fieldv))
                        continue
                    end
                end
            end
        ...
        if use.expr.head === :(=)
            assign_update_todo!(ctx, use.expr)
        end
    end
    # Slot
    if !key.second
...
function split_disjoint_assign!(ctx::AllocOptContext, info, key)
    ...
    for use in info.uses
        ...
        ty = exprtype(usex, ctx.sv.src, ctx.sv.mod)
        if isa(ty, Const)
            replace_use_expr_with!(ctx, use, maybe_quoting(ty.val))
        elseif isa(ty, Conditional)
            ...
            if isempty(vars)
                replace_use_expr_with!(ctx, use, maybe_quoting(false))
            else
                var = vars[1]
...
function show(io::IO, use::ValueUse)
    if isdefined(use, :expr)
...
function show(io::IO, def::ValueDef)
    print(io, "ValueDef(assign=`", def.assign, "`, stmts=..., stmtidx=", def.stmtidx, ")")
end
function show(io::IO, info::ValueInfo)
    println(io, "ValueInfo:")
...
function show(io::IO, info::ValueInfoMap)
...
function structinfo_constant(ctx::AllocOptContext, @nospecialize(v), vt::DataType)
    nf = fieldcount(vt)
    if vt <: Tuple
        si = StructInfo(Vector{Any}(nf), Symbol[], vt, false)
    else
        si = StructInfo(Vector{Any}(nf), fieldnames(vt), vt, false)
    end
    for i in 1:nf
        if isdefined(v, i)
            si.defs[i] = maybe_quoting(getfield(v, i))
        else
...
structinfo_tuple(ex::Expr) = StructInfo(ex.args[2:end], Symbol[], Tuple, false)
function structinfo_new(ctx::AllocOptContext, ex::Expr, vt::DataType)
    nf = fieldcount(vt)
    si = StructInfo(Vector{Any}(nf), fieldnames(vt), vt, vt.mutable)
...
function split_struct_alloc!(ctx::AllocOptContext, info, key)
    ...
    #     Field that are unknown for all defs are also allowed.
    #
    # * `getfield`
    #
    ...
    if ndef == 1
        split_struct_alloc_single!(ctx, info, key, min_nf, has_ccall, has_setfield_undef)
        if !key.second
            ctx.sv.src.slotflags[key.first] = 0
        end
        delete!(ctx.infomap, key)
...
function split_struct_alloc_single!(ctx::AllocOptContext, info, key, nf, has_ccall,
                                    has_setfield_undef)
    ...
    # expressions.
    for i in 1:nf
        has_fld_use = haskey(ctx.all_fld, i) || (!isempty(si.names) &&
                                                 haskey(ctx.all_fld, si.names[i]))
        has_def = isassigned(si.defs, i)
        has_setfld_use = false
        field_typ = has_def ? widenconst(exprtype(si.defs[i], ctx.sv.src, ctx.sv.mod)) : Union{}
        if has_fld_use && !isempty(ctx.setfield_typ)
            if haskey(ctx.setfield_typ, i)
                has_setfld_use = true
                field_typ = Union{field_typ,ctx.setfield_typ[i]}
            end
            if !isempty(si.names) && haskey(ctx.setfield_typ, si.names[i])
                has_setfld_use = true
                field_typ = Union{field_typ,ctx.setfield_typ[si.names[i]]}
            end
        end
        need_ccall_root = has_ccall && !isbits(field_typ)
        local var_slot
        if !has_def
            # If there's no direct use of the field
            # (and we know that there's no use in ccall root) we can ignore this field
            ...
            flag_slot = add_slot!(ctx.sv.src, Bool, true, :field_flag)
            ctx.sv.src.slotflags[end] = Slot_StaticUndef
            flag_vars[i] = flag_slot
            add_allocopt_todo(ctx, flag_slot)

            var_slot = add_slot!(ctx.sv.src, field_typ, false, :field_val)
            ctx.sv.src.slotflags[end] = Slot_StaticUndef
            vars[i] = var_slot
        ...
        elseif has_setfld_use
            ...
            orig_def = si.defs[i]
            isconst, cv = get_const_exprarg(orig_def, ctx.sv)
            if isconst
                vars[i] = maybe_quoting(cv)
                # Constants don't need ccall roots
                continue
            end
            if has_fld_use || need_ccall_root
                need_assign = true
                if is_ssa
                ...
                if isa(orig_def, SSAValue)
                    add_allocopt_todo(ctx, orig_def)
                else
                    isa(orig_def, Slot) && add_allocopt_todo(ctx, orig_def)
                    push!(def_exprs, orig_def)
                    scan_expr_use!(ctx.infomap, def_exprs, length(def_exprs),
                                   orig_def, ctx.sv.src)
                end
            end
        end
        if need_ccall_root
            push!(ccall_vars, var_slot)
        end
    end
    ...
    for use in info.uses
        ...
        head = expr.head
        if head === :isdefined
            replace_use_expr_with!(ctx, use, maybe_quoting(true))
            continue
        elseif head === :foreigncall
            # Splicing in the new ccall root variables without moving existing other roots
            nccall_vars = length(ccall_vars)
            if nccall_vars == 0
                ...
                if nccall_vars > 1
                    ...
                    resize!(expr.args, old_nargs + nccall_vars - 1)
                    for i in 2:nccall_vars
                        cvar = ccall_vars[i]
        ...
        elseif use_kind == 1
            ...
            elseif haskey(ctx.setfield_typ, fld) || (!isempty(si.names) &&
                ...
                not_flag_expr = :($not_flag = $not_flag_val)
                after_lbl = genlabel(ctx.sv)
                branch = Expr(:gotoifnot, not_flag, after_lbl.label)
                throw_err = Expr(:call, GlobalRef(Core, :throw), UndefRefError())
                throw_err.typ = Union{}
                stmts = Any[not_flag_expr, branch, throw_err, after_lbl]
                add_use(ctx.infomap, flag_slot, ValueUse(stmts, 1, not_flag_val, 2))
                add_def(ctx.infomap, not_flag, ValueDef(not_flag_expr, stmts, 1))
                add_use(ctx.infomap, not_flag, ValueUse(stmts, 2, branch, 1))
                stmt = use.stmts[use.stmtidx]::Expr
                if stmt !== expr
...
function verify_value_infomap(ctx::AllocOptContext)
    ...
    for i in 1:length(infomap.ssas)
        ...
        for def in info.defs
            ...
            @assert assign.head === :(=)
            @assert def.stmts[def.stmtidx] === assign
            @assert assign.args[1] === ssav
        end
        @assert ndef <= 1
        nuse = 0
        for use in info.uses
            ...
            if !isdefined(use, :expr)
                ...
                expr = use.expr
                @assert expr.args[use.exidx] === ssav
                if stmt === expr
                    if expr.head === :(=)
                        @assert use.exidx != 1
                    end
                elseif expr.head === :&
                    ...
                    @assert ccall_expr.head === :foreigncall
    ...
    for i in 1:length(infomap.slots)
        ...
        for def in info.defs
            ...
            @assert def.stmts[def.stmtidx] === assign
            if isa(assign, NewvarNode)
                @assert var_has_undef(ctx.sv.src, i)
                @assert assign.slot === slotv
            else
                ndef += 1
                @assert assign.head === :(=)
                @assert assign.args[1] === slotv
            end
        end
        ...
        for use in info.uses
            ...
            if !isdefined(use, :expr)
                @assert symequal(stmt, slotv)
            else
                expr = use.expr
                @assert symequal(expr.args[use.exidx], slotv)
                if stmt === expr
                    if expr.head === :(=)
                        @assert use.exidx != 1
                    end
...
function verify_value_infomap_rescan(ctx::AllocOptContext, stmts, seen, in_methods, all_stmts)
    ...
    for i in 1:length(stmts)
        ...
        for j in 1:length(eargs)
            ...
            if j == 1 && head === :method
                if isa(ea, Slot)
                    @assert haskey(in_methods, SlotNumber(slot_id(ea)))
                    continue
                end
            end
            if isccall && isa(ea, Expr)
